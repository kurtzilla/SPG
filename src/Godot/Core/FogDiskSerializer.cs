...
public partial class FogDiskSerializer : Node
{
	...
	public async Task<bool> SaveStateExplicitAsync(string serializedPayload)
	{
		...
		try
		{
			...
			await File.WriteAllTextAsync(_tempPath, serializedPayload, Encoding.UTF8).ConfigureAwait(false);
	...
	public async Task<string> LoadStateFromDiskAsync()
	{
		...
		try
		{
			return await File.ReadAllTextAsync(_masterPath, Encoding.UTF8).ConfigureAwait(false);
		}
		catch (Exception ex)
		{
			GD.PrintErr($"FogDiskSerializer: load failed — {ex.Message}");
